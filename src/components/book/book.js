...
const BookTrip = (props) => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const calendarData = await axios.get(`https://www.googleapis.com/calendar/v3/calendars/nearfallfishingcharters@gmail.com/events?maxResults=2500&key=${process.env.REACT_APP_GOOGLE_API_KEY}`);
            ...
            if (source === 'calendar') {
                ...
                const filterByName = calendarData.data.items.filter((x) => {
                ...
                const filterTripData = Object.keys(trips).filter((x) => { 
                        ...
                        <FormGroup>
                            ...
                            <Input type="select" name="select" id="exampleSelect">
                             {tripDate && <option>{tripDate}</option>}
                              {allTrips && allTrips.map((x) => {
                                  const date = new Date(x.start.dateTime);
                                  const formattedDate = date.toUTCString();
                                  return <option value={Date.parse(date)}>{formattedDate}</option>
                              })}
                            </Input>
                        </FormGroup>
                        <FormGroup>
                            <Label for="name">Name</Label>
                            <Input type="text" name="name" id="name" placeholder="Enter your name" onChange={onInputUpdate}/>
                        </FormGroup>
                        <FormGroup>
                            <Label for="email">Email</Label>
                            <Input type="email" name="email" id="email" placeholder="Enter your email" onChange={onInputUpdate}/>
                        </FormGroup>
                        <FormGroup>
                            <Label for="phone">Phone Number</Label>
                            <Input type="number" name="phone" id="phone" placeholder="Enter your phone number" onChange={onInputUpdate}/>
                        </FormGroup>
                        <FormGroup>
                            ...
                            <Styled.Text>
                                Total Price: ${tripDetails?.price.deposit + tripDetails?.price.balance}
                            </Styled.Text>
                            </Styled.ContainerVert>
